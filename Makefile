...
mixedtype:
	...
	@# INJECTION (proves the Q5_K column is really checked): Q5_K reuses the Q4_K
	@# header + min-subtract arms and differs ONLY in taking a 5-bit code off w_hp.
	@# The one-line way to get it silently wrong is to let WT_Q5K join the P3
	@# pass-through list and skip the dmin*m subtract -- widths all still match, no
	@# error, just wrong weights. -DINJ_Q5K_NOMIN makes exactly that edit; the gate
	@# MUST fail with it, or the Q5_K columns are riding along unchecked.
	@$(IVERILOG) $(IFLAGS) -DINJ_Q5K_NOMIN -o $(BUILD_DIR)/glm_matmul_mixed_inj test/glm_matmul_mixed_tb.v src/glm_matmul_q4k.v 2>/dev/null \
	    || { echo "FAILED: Q5_K injection compile"; exit 1; }
	@if $(VVP) $(BUILD_DIR)/glm_matmul_mixed_inj 2>/dev/null | grep -q 'ALL [0-9]* TESTS PASSED'; then \
	    echo "FAILED: Q5_K injection PASSED -- dropping the dmin*m subtract should break Q5_K; the Q5_K columns prove nothing"; exit 1; \
	  else \
	    echo "[glm_matmul_mixed_INJECT_q5k_nomin] injection correctly FAILED (the Q5_K min subtract is load-bearing and really checked)"; \
	  fi
...
# ---- GLM-5.3-Flash executable reference for the UNBUILT blocks -------------
# KDA linear attention (34/45 layers), the mHC hyper-connection residual and the
# clamped SwiGLU have no RTL yet. Writing that RTL from config.json alone would
# be guessing -- the config publishes hc_sinkhorn_iters and swiglu_limit, not the
# ORDER of operations, and each one has a detail a plausible guess gets wrong:
#   * the SwiGLU clamp is ASYMMETRIC (gate upper-only, up both ways);
#   * FLA's l2norm puts eps INSIDE the sqrt, and only q gets the 1/sqrt(Dk) scale;
#   * KDA decays the state BEFORE reading kv, and the delta rule writes (v - kv);
#   * the mHC Sinkhorn loop is one column pass THEN iters-1 (row,col) pairs, and
#     `post` is 2*sigmoid so it spans [0,2];
#   * the forget gate's fp32 sigmoid saturates, so -5.0*0.0 == -0.0 -- signed zero
#     is part of the contract a bitwise RTL gate will hold us to.
# tools/glm53_flash_ref.py is that spec, transcribed from the reference
# implementation, and this gate keeps it self-consistent. It is a SPEC gate, not
# a proof: nothing here has been run against the real checkpoint's activations.
glm53f-ref:
...
# ---- mla-ref : the NoPE MLA sublayer's executable spec -----------------------
# tools/glm53f_mla_ref.py. The 11 blocks that are not KDA use MLA, and this is the
# spec the RTL is written against -- the STRUCTURAL facts config.json does not
# give you: that GLM-5.3-Flash is NoPE so `attention.key_length == kv_lora_rank`
# (both 512, where GLM-5.2's key was the latent PLUS a 64-wide rotary tail); that
# the cache holds the 512-wide LATENT, 64x smaller than per-head k+v; and that
# BOTH MLA absorptions (W_uk into q, W_uv past the softmax) are exact in real
# arithmetic but NOT in fp -- measured 517/768 and 693/768 outputs differ at the
# checkpoint's widths, so the RTL's choice of form is a numerical decision the
# golden has to match rather than a free optimisation.
mla-ref:
	@printf '[%s] ' "mla-ref"; python3 tools/glm53f_mla_ref.py \
	    || { echo "FAILED: mla-ref"; exit 1; }

# ---- mla-score : the absorbed-latent MLA inner loop --------------------------
# src/glm53f_mla_score.v -- the part that runs once per CACHED TOKEN, so it is the
# whole decode cost of the 11 MLA blocks. BITWISE against the reference, not a
# tolerance, because every piece it uses already has a bit-exact Python twin
# (rmsnorm_unit at LANES=1, glm_softmax, glm_fp.vh): the unit adds a new dataflow
# and no new numerics.
#   The corpus MUST contain windows shorter than SMAX -- a unit that never wrote
# the -inf pad would pass every full-window test -- and the generator asserts both
# cases are present. Each must-fail leg was measured against the golden before
# being written: NOPAD 48 of 96 ctx elements move (all in the padded windows),
# NORESCALE 80, HEAD0_PROBS 40.
mla-score:
	@mkdir -p $(BUILD_DIR)
	@printf '[%s] ' "glm53f_mla_score_gen"; python3 tools/glm53f_mla_score_gen.py --selftest \
	    || { echo "FAILED: glm53f_mla_score_gen self-test"; exit 1; }
	@python3 tools/glm53f_mla_score_gen.py 6 $(BUILD_DIR)/glm53f_mla_score_vec.txt >/dev/null
	@$(IVERILOG) $(IFLAGS) -DTB_VEC='"$(BUILD_DIR)/glm53f_mla_score_vec.txt"' \
	    -o $(BUILD_DIR)/mla_score_sim test/glm53f_mla_score_tb.v src/glm53f_mla_score.v \
	    src/rmsnorm_unit.v src/glm_softmax.v src/glm_fp_pipe.v 2>/dev/null \
	    || { echo "FAILED: mla-score compile"; exit 1; }
	@printf '[%s] ' "mla_score"; $(VVP) $(BUILD_DIR)/mla_score_sim | grep -E 'ALL [0-9]+ TESTS PASSED' \
	    || { echo "FAILED: mla_score"; exit 1; }
	@for inj in INJ_MLAS_NOPAD INJ_MLAS_NORESCALE INJ_MLAS_HEAD0_PROBS; do \
	    $(IVERILOG) $(IFLAGS) -D$$inj -DTB_VEC='"$(BUILD_DIR)/glm53f_mla_score_vec.txt"' \
	        -o $(BUILD_DIR)/mla_score_inj test/glm53f_mla_score_tb.v src/glm53f_mla_score.v \
	        src/rmsnorm_unit.v src/glm_softmax.v src/glm_fp_pipe.v 2>/dev/null; \
	    if $(VVP) $(BUILD_DIR)/mla_score_inj 2>/dev/null | grep -q 'ALL [0-9]* TESTS PASSED'; then \
	        echo "FAILED: mla-score $$inj PASSED -- that trap is not actually checked"; exit 1; \
	    else \
	        echo "[mla_score_INJECT_$$inj] injection correctly FAILED"; \
	    fi; \
	done

# ---- mla-proj : the MLA projection front, Q8_0 off the shared engine ---------
# src/glm53f_mla_proj.v -- W_dq -> rmsnorm -> W_uq, the per-head fold of W_uk into
# q, and the RAW latent off W_dkv. Together with `mla-score` that is the whole
# NoPE MLA datapath for one token; what is left is wiring it at ATTN_KIND=1.
#   BITWISE, like mla-score: the GEMV is matmul_q4k_col's contract exactly
# (sequential fp32 accumulation of bf16 activation x dequantised weight, rounded
# to bf16 once), the rmsnorm is the repo's own at LANES=1, and the golden runs on
# the DEQUANTISED weights so the Q8_0 round trip is part of the INPUT -- the same
# contract `make kda-attn` uses. Q8_0 needs nothing new from the engine.
#   THE FOLD'S AXIS is the thing worth a gate. qa[h][k] reduces DOWN W_uk's rows
# (over d), so the store is [H][KV_LORA][QK]; reducing over the other axis has the
# SAME shape and would ship silently. The generator pins that a transpose differs,
# and that heads do not mix.
#   Three must-fail legs, each justified by a self-test measurement rather than
# hoped for: NO_QNORM (the c_q rmsnorm is live -- q differs without it),
# FOLD_HEAD0 (heads do not mix normally), NORM_CKV (the latent is emitted RAW --
# normalising here too applies rmsnorm twice; downstream that is only 2.8e-06
# relative and invisible to shape checks, which is how it got into the reference
# before being caught, so it gets a leg of its own here where it is loud).
mla-proj:
	@mkdir -p $(BUILD_DIR)
	@printf '[%s] ' "glm53f_mla_proj_gen"; python3 tools/glm53f_mla_proj_gen.py --selftest \
	    || { echo "FAILED: glm53f_mla_proj_gen self-test"; exit 1; }
	@python3 tools/glm53f_mla_proj_gen.py 4 $(BUILD_DIR)/glm53f_mla_proj_vec.txt >/dev/null
	@$(IVERILOG) $(IFLAGS) -DTB_VEC='"$(BUILD_DIR)/glm53f_mla_proj_vec.txt"' \
	    -o $(BUILD_DIR)/mla_proj_sim test/glm53f_mla_proj_tb.v src/glm53f_mla_proj.v \
	    src/glm_matmul_q4k.v src/rmsnorm_unit.v src/glm_fp_pipe.v 2>/dev/null \
	    || { echo "FAILED: mla-proj compile"; exit 1; }
	@printf '[%s] ' "mla_proj"; $(VVP) $(BUILD_DIR)/mla_proj_sim | grep -E 'ALL [0-9]+ TESTS PASSED' \
	    || { echo "FAILED: mla_proj"; exit 1; }
	@for inj in INJ_MLAP_NO_QNORM INJ_MLAP_FOLD_HEAD0 INJ_MLAP_NORM_CKV; do \
	    $(IVERILOG) $(IFLAGS) -D$$inj -DTB_VEC='"$(BUILD_DIR)/glm53f_mla_proj_vec.txt"' \
	        -o $(BUILD_DIR)/mla_proj_inj test/glm53f_mla_proj_tb.v src/glm53f_mla_proj.v \
	        src/glm_matmul_q4k.v src/rmsnorm_unit.v src/glm_fp_pipe.v 2>/dev/null; \
	    if $(VVP) $(BUILD_DIR)/mla_proj_inj 2>/dev/null | grep -q 'ALL [0-9]* TESTS PASSED'; then \
	        echo "FAILED: mla-proj $$inj PASSED -- that trap is not actually checked"; exit 1; \
	    else \
	        echo "[mla_proj_INJECT_$$inj] injection correctly FAILED"; \
	    fi; \
	done
...
# ---- kda-layer : ONE Kimi Delta Attention layer, 34 of GLM-5.3-Flash's 45 -------
# src/glm53f_kda_layer.v composes the four proven non-GEMV units and OWNS the two
# pieces of state that make KDA not a drop-in for mla_attn_q4k: the [H,DK,DV]
# recurrence (fixed size -- NOT a growing KV cache) and the [3*H*DK,K-1] conv
# history.  The nine projections are answered behaviourally by the TB, which
# isolates the LAYER's job from the GEMV engine that q4k/mixedtype already gate.
#   Bounds are COMPOSED from the units' own published numbers, not read off the
# DUT: rel 6 % because 4.3e measures a KDA layer at 1-3 % (glm_act's bf16 sigmoid
# on the gate path), abs 0.03 because kda_onorm_step is gated at abs 0.004 and
# o_proj sums H*DV of them.
kda-layer:
	@mkdir -p $(BUILD_DIR)
	@printf '[%s] ' "glm53f_kda_layer_gen"; python3 tools/glm53f_kda_layer_gen.py --selftest \
	    || { echo "FAILED: glm53f_kda_layer_gen self-test"; exit 1; }
	@python3 tools/glm53f_kda_layer_gen.py 8 $(BUILD_DIR)/glm53f_kda_layer_vec.txt >/dev/null
	@$(IVERILOG) $(IFLAGS) -DTB_VEC='"$(BUILD_DIR)/glm53f_kda_layer_vec.txt"' -o $(BUILD_DIR)/kda_layer_sim \
	    test/glm53f_kda_layer_tb.v src/glm53f_kda_layer.v src/kda_conv_step.v src/kda_gate_step.v src/kda_recur.v \
	    src/kda_onorm_step.v src/glm_act.v src/rmsnorm_unit.v src/glm_fp_pipe.v 2>/dev/null \
	    || { echo "FAILED: kda-layer compile"; exit 1; }
	@printf '[%s] ' "kda_layer"; $(VVP) $(BUILD_DIR)/kda_layer_sim | grep -E 'ALL [0-9]+ TESTS PASSED' \
	    || { echo "FAILED: kda_layer"; exit 1; }
	@for inj in INJ_KDAL_NO_STATE INJ_KDAL_CONV_NOHIST INJ_KDAL_QK_SWAP INJ_KDAL_GATE_ORDER; do \
	    $(IVERILOG) $(IFLAGS) -D$$inj -DTB_VEC='"$(BUILD_DIR)/glm53f_kda_layer_vec.txt"' \
	        -o $(BUILD_DIR)/kda_layer_inj test/glm53f_kda_layer_tb.v src/glm53f_kda_layer.v src/kda_conv_step.v src/kda_gate_step.v src/kda_recur.v \
	    src/kda_onorm_step.v src/glm_act.v src/rmsnorm_unit.v src/glm_fp_pipe.v 2>/dev/null; \
	    if $(VVP) $(BUILD_DIR)/kda_layer_inj 2>/dev/null | grep -q 'ALL [0-9]* TESTS PASSED'; then \
	        echo "FAILED: kda-layer $$inj PASSED -- that trap is not actually checked"; exit 1; \
	    else \
	        echo "[kda_layer_INJECT_$$inj] injection correctly FAILED"; \
	    fi; \
	done
...
# ---- dec-block : KDA attention wired INSIDE the two-site mHC block -------------
# src/glm53f_decoder_block.v -- the first module where a real sublayer lives inside
# the hyper-connection. `make hc-block` checks the two mHC sites with stubs;
# `make kda-attn` checks the KDA sublayer alone; this checks the composition, with
# the recurrence and conv history advancing across the attention site.
#   The FFN site stays a handshake ON PURPOSE: GLM-5.3-Flash's dense FFN is Q8_0
# (`blk.N.ffn_{gate,up,down}` [scan]) and its MoE experts are a Q4_K/Q5_K/Q6_K mix,
# so swiglu_expert_q4k -- 4 bits per lane -- cannot carry either. Hanging it off
# anyway to make the block look finished is the silent-wrong-weights failure this
# repo builds must-fail pairs against. A Q8_0 clamped SwiGLU is the next sibling.
#   ATTN_KIND=1 (MLA) $fatals: mla_attn_q4k is not wired into the site yet, and
# elaborating as if it were would be a top that lies.
dec-block:
	@mkdir -p $(BUILD_DIR)
	@printf '[%s] ' "glm53f_decoder_block_gen"; python3 tools/glm53f_decoder_block_gen.py --selftest \
	...
	@$(IVERILOG) $(IFLAGS) -DTB_VEC='"$(BUILD_DIR)/glm53f_decoder_block_vec.txt"' \
	    ...
	    src/kda_conv_step.v src/kda_gate_step.v src/kda_recur.v src/kda_onorm_step.v \
	    src/glm_act.v src/glm_matmul_q4k.v 2>/dev/null \
	...
	@for inj in INJ_DBLK_SITE_SWAP INJ_DBLK_NO_KDA; do \
	    ...
	    src/kda_conv_step.v src/kda_gate_step.v src/kda_recur.v src/kda_onorm_step.v \
	    src/glm_act.v src/glm_matmul_q4k.v 2>/dev/null; \
	    if $(VVP) $(BUILD_DIR)/dec_block_inj 2>/dev/null | grep -q 'ALL [0-9]* TESTS PASSED'; then \
	        echo "FAILED: dec-block $$inj PASSED -- that trap is not actually checked"; exit 1; \
	    else \
	        echo "[dec_block_INJECT_$$inj] injection correctly FAILED"; \
	    fi; \
	done
...
#   INJ_SWQ8_NOCLAMP removes the clamp, proving the golden really encodes it;
#   INJ_SWQ8_Q4K_TYPE leaves w_type at Q4_K, which is also what an UNDRIVEN w_type
#   reads as, so "forgot to drive it" and "drove it wrong" look identical.
#   INJ_SWQ8_SYMCLAMP is deliberately NOT in that list. Measured, the symmetric-gate
# reading moves the result 0.25 against a 2.9 tolerance -- inside it, because the
# tolerance is forced by glm_act's polynomial silu and no exact model of that
# polynomial exists in Python. The ASYMMETRY is gated on a tighter slice by `make
# swiglu`'s own INJ_SWIGLU_SYMCLAMP leg. A gate that cannot fail is worse than none.
#   The generator asserts the corpus drives BOTH clamp bounds, or the leg would be
# vacuous regardless of the injections.
swiglu-q8:
	@mkdir -p $(BUILD_DIR)
	@printf '[%s] ' "glm53f_swiglu_q8_gen"; python3 tools/glm53f_swiglu_q8_gen.py --selftest \
